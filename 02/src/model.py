...
class LogisticRegressionModel:
    def __init__(self, input_dim: int, output_dim: int):
        self.W = np.random.uniform(low=-0.08, high=0.08, size=(input_dim, output_dim)).astype('float32')
        ...
        self.optimizer = Optimizer(method="adam", shape_W=self.W.shape, shape_b=self.b.shape)
    ...
    def load_data(self, data_path: str) -> None:
        ...
        self.x_train, self.x_valid, self.y_train, self.y_valid = train_test_split(x_train, y_train, test_size=0.1)
...
class Optimizer:
    def __init__(self, method="sgd", shape_W=None, shape_b=None, beta=0.9, beta1=0.9, beta2=0.999, epsilon=1e-8):
        self.method = method
        self.beta = beta
        self.beta1 = beta1
        self.beta2 = beta2
...
class Scheduler:
    def __init__(self, initial_lr: float, decay_rate: float = 0.95, decay_step: int = 10,
                 plateau_patience: int = 10, plateau_factor: float = 0.5, min_lr: float = 1e-6):
...
if __name__ == "__main__":
    ...
    for epoch in range(10000):
        ...
        if epoch % 10 == 0:
            acc = accuracy_score(np.argmax(model.y_valid, axis=1), np.argmax(y_pred, axis=1))
    ...
    submission.to_csv("./02/submission/submission_pred.csv", header=True, index_label='id')